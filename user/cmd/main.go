...
func main() {
    ...
    // 5. Apply filtering (Remove noisy processes and self)
    filteredRaw := pipeline.Filter(rawEvents)

    // 6. Transform Stage: Convert ebpf.Event to event.Event
    transformedEvents := make(chan event.Event, 500)
    go func() {
        defer close(transformedEvents)
        for e := range filteredRaw {
            // Use your convert.go logic
            transformedEvents <- ebpf.ToEvent(e)
        }
    }()

    // 7. Hand over to the final pipeline runner (with Tracker & Lineage)
